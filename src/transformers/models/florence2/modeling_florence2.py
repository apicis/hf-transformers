...
class Florence2LanguageForConditionalGeneration(Florence2LanguagePreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: Florence2LanguageConfig, use_negative=False):
        super().__init__(config)
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if self.use_negative:
                loss_dict = self.loss_fct(logits=lm_logits, labels=labels, output_dict=return_dict)
                loss_cap = loss_dict["caption_loss"]
                loss_neg = loss_dict["negative_loss"]
                masked_lm_loss = loss_cap + loss_neg
            else:
...
class Florence2ForConditionalGeneration(Florence2PreTrainedModel):
    def __init__(self, config: Florence2Config, use_negative: bool):
        super().__init__(config)
        ...
        self.use_negative = use_negative
        print(f"{use_negative=}")
        language_model = Florence2LanguageForConditionalGeneration(config=config.text_config, use_negative=use_negative)
    ...
    def forward(
        ...
        loss = outputs.loss
        if self.use_negative:
            loss_cap = outputs.loss_cap
            loss_neg = outputs.loss_neg
        if not return_dict:
